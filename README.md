...
The R3 runtime evidence guard protects the intake boundary for those future runtime logs. The current package is `R3_RUNTIME_EVIDENCE_GUARD_READY_AWAITING_CAPTURE` with `35` slots, `70` required runtime events, `0` accepted runtime slots, `35` pending slots, `23/23` negative controls detected and `6/6` fixture-only positive controls accepted; this rejects projected proofs, invalid timestamps, duplicated event IDs, missing field state, missing gate delta, non-finite tension values, missing or mismatched entities, mismatched slot/original event identity, broken no-tick causal pairs, missing proof hashes, missing native proof flags, invalid native source format, weak authority, missing target heuristic rules and non-no-tick prechecks before cutover.
