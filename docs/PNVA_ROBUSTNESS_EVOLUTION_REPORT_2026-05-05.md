...
This closes a contract drift risk. Before PNVA accepts final runtime JSONL, the R3 slot IDs, original event IDs, guard controls, no-tick precheck templates, commit templates, exact role state transitions, exact event-type binding, exact pair cardinality, exact runtime event count, native proof markers, source location uniqueness, source-line order, source sanitization, proof identity binding and native source format must agree as one validated contract.
