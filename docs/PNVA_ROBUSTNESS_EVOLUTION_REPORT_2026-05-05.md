...
This closes a contract drift risk. Before PNVA accepts final runtime JSONL, the R3 slot IDs, original event IDs, causal-chain uniqueness per slot, guard controls, no-tick precheck templates, commit templates, exact role state transitions, exact event-type binding, exact pair cardinality, exact runtime event count, native proof markers, source location uniqueness, JSONL-line order, source-line order, per-file source-line monotonicity, source sanitization, proof identity binding and native source format must agree as one validated contract.
